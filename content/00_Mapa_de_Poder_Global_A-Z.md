...
```mermaid
graph TD
    classDef level1 fill:#2e003e,stroke:#fff,stroke-width:2px,color:#fff;
    classDef tierA fill:#8b0000,stroke:#fff,stroke-width:2px,color:#fff;
    classDef tierB fill:#00008b,stroke:#fff,stroke-width:1px,color:#fff;
    classDef tierC fill:#4a4a4a,stroke:#fff,stroke-width:1px,color:#fff;
    classDef flow fill:none,stroke:#0f0,stroke-width:1px,color:#0f0,stroke-dasharray: 5 5;

    subgraph Nivel_1 [NIVEL 1: COOPERATIVO - Cartel Transnacional]
        direction TB
        BIS(BIS: Reglas del Sistema):::level1
        BlackRock(Capital Sistémico / Aladdin):::level1
        Dynasties(Tier A: Dueños / Dinastías):::level1
    end

    subgraph Nivel_2 [NIVEL 2: COMPETITIVO - Teatro de Operaciones]
        direction TB
        TierB(Tier B: Ejecutores / Inteligencia-Banca Central):::tierB
        TierC(Tier C: Narrativa / Políticos-Medios):::tierC
    end

    %% Flujos de Poder
    BIS -->|Basilea III/IV| TierB
    BlackRock -->|ESG / Inversión| TierB
    Dynasties -->|Fondos / Think Tanks| TierB
    
    TierB -->|Ingeniería Social| TierC
    TierB -->|Financiamiento| TierC
    
    TierC -.->|Distracción Social| Nivel_2
    TierB -.->|Extracción de Riqueza| Nivel_1
```
